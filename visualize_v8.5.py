...
from gaussian_rendererforview.__init__11 import render
...
class MainWindow(QMainWindow):
    ...
    def update_frame(self, ):
        if not self.imgQueue.empty():
            ...
            q_imagescaled = q_image.scaled(self.w * 4 / 3, self.h * 4 / 3, Qt.AspectRatioMode.KeepAspectRatio)
...
def renderimg(config, scene, imgQueue, get_angles, smplQueue):
    frame_count = 0
...
def main(config):
    ...
    t1 = threading.Thread(target=renderimg, args=(config, None, mainWindow.imgQueue, mainWindow.get_angles, mainWindow.smplQueue))
    t1.start()
    # t1.join()
    time.sleep(0.001)
    t2 = threading.Thread(target=getsmpl_net, args=('', 8082, mainWindow.smplQueue))
    t2.daemon = True
    t2.start()
    time.sleep(0.001)
