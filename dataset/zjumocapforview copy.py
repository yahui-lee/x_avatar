...
class ZJUMoCapDatasetforview(Dataset):
    ...
    def getitem(self, idx, data_dict=None):
        ...
        # === 读取 SMPL 模型参数
        model_dict = np.load(model_file)
        trans = torch.from_numpy(model_dict['trans']).to(device)
        root_orient = torch.from_numpy(model_dict['root_orient']).to(device)
        pose_body = torch.from_numpy(model_dict['pose_body']).to(device)
        pose_hand = torch.from_numpy(model_dict['pose_hand']).to(device)
        betas = torch.from_numpy(model_dict['betas']).to(device)

        pose = torch.cat([root_orient, pose_body, pose_hand], dim=-1).reshape(-1, 3)  # (24, 3)
        ...
        _, bone_transforms, _ = batch_rigid_transform(pose_mat_full_, J, parents, dtype=torch.float32)
