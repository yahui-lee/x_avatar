...
class SMPL(nn.Module):
    ...
    def forward(
        ...
        body_transl: Optional[Tensor] = None,
        return_verts=True,
        ...
        **kwargs
        ...
        if v_shape is None:
            v_shape = torch.zeros_like(self.v_template)
        if v_pose is None:
            v_pose = torch.zeros_like(self.v_template)

        vertices, joints, vertices_transform, joints_transform, shape_offsets, pose_offsets = lbs(betas, full_pose, v_shape, v_pose, self.v_template,
...
class SMPLX(SMPLH):
    ...
    def forward(
        ...
        v_shape: Optional[Tensor] = None,
        v_pose: Optional[Tensor] = None,
        body_trans: Optional[Tensor] = None,
        left_hand_trans: Optional[Tensor] = None,
        right_hand_trans: Optional[Tensor] = None,
        jaw_trans: Optional[Tensor] = None,
        leye_trans: Optional[Tensor] = None,
        reye_trans: Optional[Tensor] = None,
        return_verts: bool = True,
        return_full_pose: bool = False,
        pose2rot: bool = True,
        ...
        **kwargs
        ...
        body_trans = body_trans if body_trans is not None else torch.zeros_like(self.body_pose)
        left_hand_trans = left_hand_trans if left_hand_trans is not None else torch.zeros_like(self.left_hand_mean)
        right_hand_trans = right_hand_trans if right_hand_trans is not None else torch.zeros_like(self.right_hand_mean)
        jaw_trans = jaw_trans if jaw_trans is not None else torch.zeros_like(self.jaw_pose)
        leye_trans = leye_trans if leye_trans is not None else torch.zeros_like(self.leye_pose)
        reye_trans = reye_trans if reye_trans is not None else torch.zeros_like(self.reye_pose)
        ...
        full_trans = torch.cat([body_trans.reshape(-1, self.NUM_BODY_JOINTS, 3),
                                jaw_trans.reshape(-1, 1, 3),
                                leye_trans.reshape(-1, 1, 3),
                                reye_trans.reshape(-1, 1, 3),
                                left_hand_trans.reshape(-1, 15, 3),
                                right_hand_trans.reshape(-1, 15, 3)],
                                dim=1).reshape(-1, 162)
        ...
        vertices, joints, vertices_transform, joints_transform, shape_offsets, pose_offsets = lbs(shape_components, full_pose, v_shape, v_pose, self.v_template,
                               shapedirs, self.posedirs,
                               self.J_regressor, self.parents,
                               self.lbs_weights, pose2rot=pose2rot, trans=full_trans,
                               )
