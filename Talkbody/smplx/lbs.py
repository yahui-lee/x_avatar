...
def lbs(
    ...
    v_shaped = v_template + shape_offsets + v_shape
    ...
    if trans is not None:
        trans = trans.view(batch_size, -1, 3)

    v_posed = pose_offsets + v_shaped + v_pose
    # 4. Get the global joint location
    J_transformed, A = batch_rigid_transform(rot_mats, J, parents, joints_delta=trans, dtype=dtype)
    ...
    return verts, J_transformed, T, A, shape_offsets, pose_offsets
...
def batch_rigid_transform(
    ...
    if joints_delta is not None:
        rel_joints[:, 1:] = rel_joints[:, 1:] + joints_delta.unsqueeze(-1)
