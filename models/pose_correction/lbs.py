...
def lbs(betas, pose, v_template, clothed_v_template, shapedirs, posedirs, J_regressor, parents,
        lbs_weights, num_joints=54, dtype=torch.float32):
...
def npbatch_rigid_transform(rot_mats, joints, parents, dtype=np.float32):
    """
    Applies a batch of rigid transformations to the joints
    ...
    # Add an extra dimension to joints for easier broadcasting (similar to torch.unsqueeze)
    joints = np.expand_dims(joints, axis=-1)

    # Create relative joints based on parents
    rel_joints = joints.copy()
    rel_joints[:, 1:] -= joints[:, parents[1:]]

    # Perform the transformation matrix calculation (similar to torch.einsum)
    R_padded = np.pad(rot_mats.reshape(-1, 3, 3), [(0, 0), (0, 1), (0, 0)], mode='constant', constant_values=0)
    t_padded = np.pad(rel_joints.reshape(-1, 3, 1), [(0, 0), (0, 1)], mode='constant', constant_values=1)
    transforms_mat = np.concatenate([R_padded, t_padded], axis=2).reshape(-1, num_joints, 4, 4)
    # Initialize the chain with the first transform
    transform_chain = [transforms_mat[:, 0]]
    
    for i in range(1, parents.shape[0]):
        curr_res = np.matmul(transform_chain[parents[i]], transforms_mat[:, i])
        transform_chain.append(curr_res)

    # Stack the transforms to form the final transformation matrix
    transforms = np.stack(transform_chain, axis=1)

    # The last column of the transformations contains the posed joints
    posed_joints = transforms[:, :, :3, 3]

    # Homogenize joints by adding a column of zeros (similar to torch.cat)
    joints_homogen = np.concatenate(
        [joints, np.zeros([batch_size, num_joints, 1, 1], dtype=dtype)], axis=2
    )

    # Compute the initial bone transforms (similar to torch.matmul)
    init_bone = np.matmul(transforms, joints_homogen)
    init_bone = np.pad(init_bone, [(0, 0), (0, 0), (3, 0), (0, 0)], mode='constant')

    # Compute the relative transforms (difference between transforms and initial bone)
    rel_transforms = transforms - init_bone
