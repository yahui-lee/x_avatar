...
class PoseCorrection(nn.Module):
    def __init__(self, config, metadata=None):
        ...
        v_template = np.load('body_models/miscx/v_templates.npz')[gender]
        lbs_weights = np.load('body_models/miscx/skinning_weights_all.npz')[gender]
        posedirs = np.load('body_models/miscx/posedirs_all.npz')[gender]
        posedirs = posedirs.reshape([posedirs.shape[0] * 3, -1]).T
        shapedirs = np.load('body_models/miscx/shapedirs_all.npz')[gender]
        J_regressor = np.load('body_models/miscx/J_regressors.npz')[gender]
        kintree_table = np.load('body_models/miscx/kintree_table.npy')
...
class DirectPoseOptimization(PoseCorrection):
    def __init__(self, config, metadata=None):
        ...
        # self.expression = nn.Embedding.from_pretrained(torch.from_numpy(expression).float(), freeze=True)
    ...
    def pose_correct(self, camera, iteration):
        ...
        # print('frame:', frame, 'camera.rots:', camera.rots, 'rots:', rots)
