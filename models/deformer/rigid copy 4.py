...
class SkinningField(RigidDeform):
    def __init__(self, cfg, metadata):
        ...
        self.hand_lbs_network = get_skinning_mlp(3, 34, cfg.hand_skinning_network)


    def get_forward_transform_body(self, xyz, tfs):
        pts_W = self.body_lbs_network(xyz)
        pts_W = self.softmax(pts_W)
        pts_W = torch.cat((pts_W[:,:],torch.zeros(pts_W.shape[0],33).cuda()),dim=1)
    ...
    def get_forward_transform_hand(self, xyz, tfs): 
        ...
        pts_W = self.softmax(pts_W)
        pts_W = torch.cat((torch.zeros(pts_W.shape[0], 18).cuda(),pts_W[:,:4],torch.zeros(pts_W.shape[0],3).cuda(),pts_W[:,4:]),dim=1)
    ...
    def get_skinning_loss(self):
        ...
        hand_pts_w = torch.cat([hand_pts_w[:,18:22],hand_pts_w[:,25:]],dim=1)
        hand_pts_w = hand_pts_w / hand_pts_w.sum(dim=-1, keepdim=True)

        body_pts_w = body_pts_w[:, :22]
        body_pts_w = body_pts_w / body_pts_w.sum(dim=-1, keepdim=True)
        ...
        skinning_loss = 0.4*hand_loss + 0.6*body_loss
