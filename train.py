...
def training(config):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        with torch.no_grad():
            ...
            # print(f"[ITER {iteration}] Loss: {loss_l1.item():.4f} L1: {loss_l1.item():.4f} SSIM: {loss_dssim.item():.4f} Perceptual: {loss_perceptual.item():.4f} Mask: {loss_mask.item():.4f} Skinning: {loss_skinning.item():.4f} AIAP_xyz: {loss_aiap_xyz.item():.4f} AIAP_cov: {loss_aiap_cov.item():.4f}")
            log_loss.update({
