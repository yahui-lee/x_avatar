...
class SMPLlayer(nn.Module):
    def __init__(self, model_path, num_shape_coeffs, num_expression_coeffs, model_type='smpl', gender='neutral', device=None,
        regressor_path=None,
        ...
        **kwargs) -> None:
        ...
        self.SHAPE_SPACE_DIM = 300
        self.EXPRESSION_SPACE_DIM = 100 
        ...
        if self.use_shape_blending:
            if self.model_type == 'smplx':
                if num_expression_coeffs == 10 and num_shape_coeffs == 10:
                    # SMPLX model with shape and expression blendshapes
                    # shape: 10, expression: 10
                    self.shapedirs = self.shapedirs[:, :, :self.num_shapes_coeffs + self.num_expression_coeffs]
                else:
                    # SMPLX model with shape and expression blendshapes
                    # shape: 100, expression: 50
                    self.shapedirs = torch.concat(
                                        (self.shapedirs[:, :, :num_shape_coeffs], 
                                        self.shapedirs[:, :, self.SHAPE_SPACE_DIM:self.SHAPE_SPACE_DIM + num_expression_coeffs]),
                                        dim=-1  # Concatenate along last dimension
                                    )

                    #self.shapedirs = self.shapedirs[:, :, :num_shape_coeffs] + self.shapedirs[:, :, self.SHAPE_SPACE_DIM:self.SHAPE_SPACE_DIM + num_expression_coeffs]
    ...
    def get_root(self, poses, shapes, return_tensor=False):
        ...
        vertices, joints, T_joints, T_vertices = lbs(shapes, poses, self.v_template,
    ...
    def forward(self, poses, shapes, Rh=None, Th=None, expression=None, 
        ...
        if return_verts or not self.use_joints:
            ...
            if not self.use_joints and not return_verts:
                vertices = joints
            # v1 = vertices[:, :self.J_regressor.shape[0], :]
            # v1 = v1[0].numpy()
            # j1 = joints[0].numpy()
            # dist = np.sqrt(pow((v1-j1),2))
            # print(dist)
            # xv = [k[0] for k in v1]
            # yv = [k[1] for k in v1]
            # zv = [k[2] for k in v1]
            ...
            # if v_template is None:
            #     v_template = self.v_template
            # vertices, jointss, T_jointss, T_verticess = self.lbs(shapes, poses, v_template,
            #                             self.shapedirs, self.posedirs,
            #                             self.J_regressor, self.parents,
            ...
            # yv2 = [k[1] for k in v2]
            # zv2 = [k[2] for k in v2]
            # ax.scatter(xv2, yv2, zv2, c='g', marker='.', s=2, linewidth=0, alpha=1, cmap='spectral')
            # ax.set_xlabel('x label')
            # ax.set_ylabel('y label')
            # ax.set_zlabel('z label')
            # plt.show()
        else:
            # torch.cat(( Rh,poses[:, 3:]), dim=1)
            vertices, joints, T_joints, T_vertices = self.lbs(shapes, torch.cat(( Rh,poses[:, 3:]), dim=1), self.j_v_template,
            ...
            # if v_template is None:
            #     v_template = self.v_template
            # vertices, jointss, T_jointss, T_verticess = self.lbs(shapes,torch.cat(( Rh,poses[:, 3:]), dim=1), v_template,
            #                             self.shapedirs, self.posedirs,
            #                             self.J_regressor, self.parents,
            #                             self.weights, pose2rot=pose2rot, dtype=self.dtype, only_shape=only_shape,
            #                             use_pose_blending=self.use_pose_blending,
